...
def alignment(path_patient, path_output, path_label, path_label_output):
    ...
    print('patient list : ', date_list)
    data_list = ['2', '3', '4', '1_OCT', '2_OCT', '3_OCT', '4_OCT'] #要對位的影像
    label_list = ['label_3', 'label_4']
    eye_list = ['R', 'L']
    if len(date_list) > 1:
        ...
        for eye in eye_list:
            if os.path.exists(path_pre_date + eye) and os.path.exists(path_post_date + eye):
                pre_date_imglist = os.listdir(path_pre_date + eye)
                post_date_imglist = os.listdir(path_post_date + eye)
                if len(pre_date_imglist) >= 10 and len(post_date_imglist) >= 10:
