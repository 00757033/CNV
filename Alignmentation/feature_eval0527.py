...
class finding():
    ...
    def preprocess(self, image):
        ...
        # image_preprocess = cv2.bilateralFilter(image_preprocess, 7, 75,75)
        # image_preprocess = cv2.createCLAHE(clipLimit=4.0, tileGridSize=(4, 4)).apply(image_preprocess)
    ...
    def find_center(self,image,image_size = (304,304),expand = 50,save_path = None,save_name = None):
        image2 = image.copy()
        ...
        # 二值化 32 255
        rst,image2 = cv2.threshold(image2, 24, 255,  cv2.THRESH_BINARY)
        ...
        # print('center',center,x1,x2,y1,y2)
        
        # cv2.rectangle(draw_image, (x1, y1), (x2, y2), (255, 0, 255), 3)
 
        t = expand
    
        if center[0] - max_radius < t:
            x1 = 0
        else:
            x1 = int(center[0] - max_radius) - t
        if center[1] - max_radius < t:
            y1 = 0
        else:
            y1 = int(center[1] - max_radius) - t
        if center[0] + max_radius > len(image[0]) - t:
            x2 = len(image[0]) 
        else:
            x2 = int(center[0] + max_radius) + t
        if center[1] + max_radius > len(image) - t:
            y2 = len(image)
        else:
            y2 = int(center[1] + max_radius) + t
            

        
        
        # if int(center[0] - max_radius) < t :
        #     x1 = int(center[0] - max_radius)
        # if int(center[1] - max_radius) < t :
        #     y1 = int(center[1] - max_radius)
        # if int(center[0] + max_radius) > len(image[0]) - t :
        #     x2 = len(image[0]) - int(center[0] + max_radius)
        # if int(center[1] + max_radius) > len(image) - t :
        #     y2 = len(image) - int(center[1] + max_radius)
        ...
        # cv2.rectangle(draw_image, (int(center[0] - max_radius-x1), int(center[1] - max_radius-y1)), (int(center[0] + max_radius+ x2), int(center[1] + max_radius+ y2)), (255, 255, 0), 3)
        draw_image2 = draw_image.copy()
        cv2.rectangle(draw_image2, (x1, y1), (x2, y2), (255, 255, 0), 3)
        ...
        if save_path :
            ...
            cv2.imwrite(save_path + save_name + '_crop.png', draw_image2)
            

        # crop_img = image[int(center[1] - max_radius)-y1:int(center[1] + max_radius)+y2, int(center[0] - max_radius)-x1:int(center[0] + max_radius)+x2]
        crop_img = image[y1:y2, x1:x2]
        
         # show the image wait 3 seconds and destroy
        # fig , ax = plt.subplots(1,3,figsize=(10,10))
        # ax[0].imshow(draw_image)
        # ax[0].set_title(str(center))
        # ax[1].imshow(image)
        # ax[1].set_title('image')
        # ax[2].imshow(crop_img)
        # ax[2].set_title(str(x1) + ' ' + str(y1) + ' ' + str(x2) + ' ' + str(y2))
        # plt.show()
        

        
        # show the image wait 3 seconds and destroy
        return  crop_img , center , x1  , y1 
    ...
    def check_H_range(self,H):
        if H is not None:
            ...
            if translation[0] > 304 // 2 or translation[0] < -304 // 2 or translation[1] > 304 // 2 or translation[1] < -304 // 2:
                ...
                return False
            if rotation_angle > 60 or rotation_angle < -60:
    ...
    def getPoints(self,img, template, method=cv2.TM_CCORR_NORMED):
    ...
    def pointMatch(self,pre_img,crop_img,center,radius, method = cv2.TM_CCOEFF_NORMED):
        elements_c, r_c = self.get_element(pre_img, crop_img, radius[0],radius[1], method)
        # 選擇最高的相關係數的位置
        e = [elements_c]
        r = [r_c]
        Relation = 0
    ...
    def feature(self, feature, matcher, distance,data_groups,expand,feature_matching= True,template_match = True,mask_folder_name = 'predict'):
        patient_dict = {}
        eyes = {'R':'OD','L':'OS'}
        image_folder = self.output_image_path+ '/1/'
        filenames = sorted(os.listdir(image_folder))
        self.output_path  = self.image_path + '/' + 'crop_'+ feature + '_' + matcher + '_' + str(distance) + feature_matching * '_feature_matching' + template_match * '_template_match' + '_expand_' + str(expand) + '/'
            ...
            if filename.endswith('.png'):
                ...
                if patient_id + '_' + eye not in patient_dict:   
                    ...
                    if pre_treatment_file != '' and post_treatment_file != '':
                        ...
                        # show histogram
                        magnification = 1
                        crop_img ,center,x0,y0 = self.find_center(post_image_gray, image_size = (int(304 * magnification), int(304 * magnification)),expand=expand ,save_path = self.output_path + 'cut' + '/',save_name = patient_id + '_' + eye + '_' + post_treatment)
                        ...
                        H = None
                        if feature_matching:
                            H = self.LK(pre_image_gray,crop_img, distance=distance, method=feature, matcher=matcher)
                        # print(H)

                        if H is not None:
                            # 平移
                            # H[0,2] -= (center[0] - int(x0))
                            # H[1,2]-=(center[1] - int(y0))
                            H[0,2] -= x0
                            H[1,2] -= y0
                            translation = (H[0, 2], H[1, 2])
                        ...
                        if template_match :    
                            if H is  None or not check:
                                ...
                                shift_x, shift_y = self.pointMatch(pre_image_gray,crop_img,center,[x0,y0], method = cv2.TM_CCOEFF_NORMED)
                        ...
                        for data in self.data_list:
                            ...
                            if os.path.exists(self.output_image_path + data + '/' + patient_id + '_' + eye + '_' + pre_treatment + '.png'):
                                ...
                                if os.path.exists(self.output_image_path + data + '/' + patient_id + '_' + eye + '_' + post_treatment + '.png'):
                                    ...
                                    patient_dict[patient_id + '_' + eye][post_treatment]['radius'] = [int(x0), int(y0)]
                        ...
                        match_par = 'crop_'+ feature + '_' + matcher + '_' + str(distance)  + feature_matching * '_feature_matching' + template_match * '_template_match' + '_expand_' + str(expand)
        ...
        print('template_match_count',template_match_count)   
        return patient_dict,template_match_count
...
if __name__ == '__main__':
    ...
    data_groups = ['CC']
    label_path = PATH_BASE  + '/' + disease + '_' + date + '_connectedComponent_bil31010_clah0708_concate34OCT'
    ...
    expands = [80]#[0,10,20,30,40,50,60,70] 
    distances = [0.8]# ,0.8,0.9
    features = ['ORB']#,'AKAZE','ORB','BRISK' ,'FREAK''KAZE','AKAZE','ORB','BRISK','BRIEF' ,'FREAK'
    matchers = ['BF']# ,'FLANN'
    # patient_list = get_data_from_txt_file('PCV.txt')
    setFolder('./record/'+ disease + '_' + date + '/') 
    feature_matching = False # False
    template_match = True
        ...
        for distance in distances:
            for feature in features:
                for matcher in matchers:
                    ...
                    find_dict,template_match_count = find.feature(feature,matcher,distance,data_groups,expand = expand,feature_matching= feature_matching,template_match = template_match)
                    find_dict = convert_float32_to_float(find_dict)
                    json_file = './record/'+ disease + '_' + date + '/'+ 'crop_'+feature + '_' + matcher + '_' + str(distance) +  feature_matching * '_feature_matching'+ template_match * '_template_match' + '_expand_' + str(expand) + '.json'
                    ...
                    eval = find.all_evaluate(image_path + 'crop' + '_' +  feature + '_' + matcher + '_' + str(distance)  + feature_matching * '_feature_matching'+ template_match * '_template_match'  + '_expand_' + str(expand))
                    csv_file = './record/'+ disease + '_' + date + '/'+ feature + '_' + matcher + '_' + str(distance) + feature_matching * '_feature_matching'+ template_match * '_template_match'  + '_expand_' + str(expand) + '.csv'
                    # tools.write_to_json_file(json_file2, eval)
                    cases = 0
                    ...
                    if not os.path.exists(aligment_file):
                        with open(aligment_file, 'w', newline='') as f:
                            ...
                            csv_writer.writerow(['feature', 'expand','cases','template_match_count','avg_mse','std_mse', 'avg_psnr', 'std_psnr', 'avg_ssim', 'std_ssim','avg_ncc','std_ncc','avg_nmi','std_nmi','avg_corr','std_corr','avg_matching_mse','std_matching_mse', 'avg_matching_psnr', 'std_matching_psnr', 'avg_matching_ssim', 'std_matching_ssim','avg_matching_ncc','std_matching_ncc','avg_matching_nmi','std_matching_nmi','avg_matching_corr','std_matching_corr'])
                    with open(aligment_file, 'a', newline='') as f:
                        csv_writer = csv.writer(f)
                        csv_writer.writerow([feature + '_' + matcher + '_' + str(distance)+ feature_matching * '_feature_matching' + template_match * '_template_match' ,expand,
                                                cases ,template_match_count,
