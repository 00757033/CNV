...
class train(): 
    ...
    def evaluateModel(self, model, test_dataset, result_path,model_path, model_name, feature,predict_threshold = 0.5,postprocess_signal = False):
        ...
        for i, item in enumerate(result):
            ...
            if postprocess_signal:
                postprocessing = crf( cv2.imread(os.path.join(img_path, test_ids[i])) , cv2.imread(os.path.join(predict_path, test_ids[i]), 0) ,os.path.join(postprocess, test_ids[i])) 
        ...
        with open(os.path.join(result_path,model_name+'_' + feature, "result.csv"), "a", newline='') as f:
            writer = csv.writer(f)
            writer.writerow(["iou", np.mean(iou), np.std(iou)])
            writer.writerow(["dice", np.mean(dice), np.std(dice)])
        ...
        return np.mean(iou), np.std(iou), np.mean(dice), np.std(dice)

    def fitModel(self, model, train_dataset, valid_dataset, epoch, lrn, model_path, model_name, feature):
        print("fitModel")
        # checkpoint
    ...
    def best_model_record(self, best_model, dataset_name, best_model_time, best_iou, best_iou_var, best_dice, best_dice_var):
        ...
        with open(os.path.join("record", "best_model.csv"), "a", newline='') as f:
            writer = csv.writer(f)
            writer.writerow([datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"), best_model, dataset_name, self.batch, self.epoch, self.learning_rate, self.predict_threshold, best_model_time, best_iou, best_iou_var, best_dice, best_dice_var])
        ...
        lines = ['----------Summary:' + best_model + '----------\n',
                ...
                'IoU:'+ str(best_iou) + ' +- ' + str(best_iou_var) + '\n',
                'Dice:'+ str(best_dice) + ' +- ' + str(best_dice_var) + '\n',
                '----------------------------------------\n']
        f.writelines(lines)
        f.close()
                ...
                for epoch in self.epochs: #每個模型訓練幾個epoch
                    for batch in self.batchs: #每個模型訓練幾個batch
                        for lrn in self.lrns: #每個模型訓練幾個learning rate
                            ...
                            model_path = os.path.join(self.model_path,floder_path,dataset_name)
                            result_path = os.path.join(self.result_path,floder_path,dataset_name)
                            ...
                            if train_signal:
                                ...
                                history, time = self.fitModel(model, train_dataset, valid_dataset,epoch, lrn ,model_path,save_model_name,count.__str__())

                                # 紀錄訓練結果
                                self.record(history, time,model_path,save_model_name,count.__str__())
                            # 預測
                            self.evaluateModel(model, dataset, result_path,model_path,save_model_name,count.__str__(),self.predict_threshold,postprocess_signal)
                            ji_score, ji_var , dice_score, dice_var = 0, 0, 0, 0
                            if ji_score > best_iou and dice_score > best_dice:
                                best_iou = ji_score
                                best_iou_var = ji_var
                                best_model = save_model_name
                                best_dice = dice_score
                                best_dice_std = dice_var
                                best_model_time = time
                            
                # self.best_model_record(best_model, dataset_name, best_model_time, best_iou, best_iou_var, best_dice, best_dice_std)
