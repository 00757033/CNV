...
#給外部執行訓練的function
    def run(self, PATH_DATASET, train_signal):  #if(train_signal==true) 才會訓練模型
        for dataset in glob.glob(r"%s/*" %PATH_DATASET):
            ...
            for data in self.datas:
                for  model_name in self.models:     
                    model_path1, result_path1 = self.mkdir(self.result_path, self.model_path, dataset_name, self.data_date, data)
                    best_model = ''
                    best_score = 0
                    best_score_var = 0 
                    for epoch in self.epochs:
                        for batch in self.batchs:
                            for lrn in self.lrns:
                                ...
                                time = 0
                                model = self.getModel(model_name, self.image_size, lrn)
...
#取得訓練模型(from UnetModel.py)
    def getModel(self, model_name, image_size, learning_rate):
        ...
        elif model_name == 'R2UNetPlusPlus':
            myModel = R2UNetPlusPlus(image_size, learning_rate)
...
#評估訓練結果
    def evaluateModel(self, myModel, model_name, feature, model_path, image_size, result_path, data_path, training_time, data, shreshold):
        print(model_path + model_name + feature + '.h5')
        myModel.load_weights(model_path + model_name + feature + '.h5')
        #儲存模型
        ...
        for result in ['predict']:
            ...
            ji_score = round(sum(iou_ji) / len(iou_ji), 2)
            ji_var   = round(np.var(iou_ji), 2)
            dc_score = round(sum(iou_dc) / len(iou_dc), 2)
            dc_var   = round(np.var(iou_dc), 2)
