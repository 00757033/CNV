...
def crf(original_image, predict_image,output_image, use_2d = True):
    ...
    if use_2d :
        ...
        d.addPairwiseBilateral(sxy=(15, 15), srgb=(13, 13, 13), rgbim=original_image,
                           compat=10,
...
if __name__ == '__main__':
    for i in os.listdir('./Result/PCV_1011/'):
        for j in os.listdir('./Result/PCV_1011/'+i):
            for k in os.listdir('./Result/PCV_1011/'+i+'/'+j):
                print('./Result/PCV_1011/'+i+'/'+j+'/'+k+'/images/')
                print('./Result/PCV_1011/'+i+'/'+j+'/'+k+'/results/')
                print('./Result/PCV_1011/'+i+'/'+j+'/'+k+'/postcrf/')
                if not os.path.exists('./Result/PCV_1011/'+i+'/'+j+'/'+k+'/postcrf/'):
                    os.makedirs('./Result/PCV_1011/'+i+'/'+j+'/'+k+'/postcrf/')

                postprocess= postprocessing('./Result/PCV_1011/'+i+'/'+j+'/'+k+'/images/','./Result/PCV_1011/'+i+'/'+j+'/'+k+'/results/','./Result/PCV_1011/'+i+'/'+j+'/'+k+'/postcrf/')
