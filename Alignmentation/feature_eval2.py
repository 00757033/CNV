...
class finding():
    ...
    def find_center(self,image,image2,image_size = (304,304),save_path = None,save_name = None):
        image_lay1 = image.copy()
        image_lay2 = image2.copy()
        
        image_lay1 = self.preprocess(image_lay1)
        image_lay2 = self.preprocess(image_lay2)
        image_lay1 = cv2.resize(image_lay1, image_size)
        image_lay2 = cv2.resize(image_lay2, image_size)

        # 二值化 32 255
        rst,image_lay1 = cv2.threshold(image_lay1, 64, 255,  cv2.THRESH_BINARY)
        rst,image_layer2 = cv2.threshold(image_lay2, 64, 255,  cv2.THRESH_BINARY)
        # 取and
        image_and = cv2.bitwise_and(image_lay1, image_layer2)
        
        image_and = cv2.bitwise_not(image_and)
        # 找到影像中 最白的最大面積的圓
        _, labels, stats, centroids = cv2.connectedComponentsWithStats(image_and, connectivity=4)
        ...
        if save_path :
            ...
            cv2.imwrite(save_path + save_name, draw_image)
            ...
            if filename.endswith('.png'):
                ...
                if patient_id + '_' + eye not in patient_dict:
                    ...
                    if pre_treatment_file != '' and post_treatment_file != '':
                        ...
                        # clean
                        mask = np.zeros_like(pre_image, dtype=np.uint8)
                        mask = cv2.cvtColor(mask, cv2.COLOR_BGR2GRAY) 

                        
                        
                        if pre_image is not None or pre_image_2 is not None :
                        ...
                        pre_image_2 = cv2.inpaint( pre_image_2, mask, 3, cv2.INPAINT_TELEA)
                        pre_image = cv2.normalize(pre_image, None, 0, 255, cv2.NORM_MINMAX)
                        pre_image_2 = cv2.normalize(pre_image_2, None, 0, 255, cv2.NORM_MINMAX)
                        ...
                        post_image_2 = cv2.inpaint( post_image_2, mask, 3, cv2.INPAINT_TELEA)
                        post_image = cv2.normalize(post_image, None, 0, 255, cv2.NORM_MINMAX)
                        post_image_2 = cv2.normalize(post_image_2, None, 0, 255, cv2.NORM_MINMAX)
                        ...
                        # show histogram
                        magnification = 1
                        crop_img ,center,radius = self.find_center(post_image,post_image_2, image_size = (int(304 * magnification), int(304 * magnification)),save_path = self.output_path + 'cut' + '/',save_name = patient_id + '_' + eye + '_' + post_treatment + '.png')
