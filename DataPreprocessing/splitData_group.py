...
class splitData():
    ...
    def splitData(self,output_name):
        print("start split data")
        data = self.splitType()
        group_shuffle_split = GroupShuffleSplit(n_splits = 2, test_size = 0.1, random_state = 3)# 42
        group_train_vail_split = GroupShuffleSplit(n_splits = 2, test_size = 0.125, random_state = 3)# 42
        ...
        for type_name , group_type_df in data:
            ...
            self.saveData(train_subset ,type_name[0],output_name, output_file_name = 'train')
            valid_subset = train_valid_subset.iloc[valid_idx]
            set_valid = set(valid_subset['group'])
            self.saveData(valid_subset ,type_name[0],output_name, output_file_name = 'valid')
            test_subset = group_type_df.iloc[test_idx]
            set_test = set(test_subset['group'])
            self.saveData(test_subset ,type_name[0],output_name, output_file_name = 'test')
    ...
    def saveData(self,data,layer ,output_name,output_file_name):
        data_dir = ['images','masks']
        layer_path = tools.get_label_path(output_name, layer)
        input_dir = os.path.join(self.path,  layer_path)
        output_dir = os.path.join(self.path, "trainset", layer_path, output_file_name)
        for  dir in data_dir:
...
if __name__ == "__main__":
    ...
    split.splitData('PCV_0918_otsu_bil_clahe4')
