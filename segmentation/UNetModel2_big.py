...
class Model():
    ...
    def standard_unit(self, inputs, filters, name = 'standard_unit'):
        ...
        leaky_relu1 = keras.layers.LeakyReLU(alpha=0.3)(bn1)
        conv2 = keras.layers.Conv2D(filters, (3, 3), kernel_initializer='he_normal', padding='same')(leaky_relu1)
        
        bn2 = keras.layers.BatchNormalization()(conv2)
        leaky_relu2 = keras.layers.LeakyReLU(alpha=0.3)(bn2)
...
class UNet(Model):
    ...
    def build_model(self, filters):
        ...
        pooling5 = keras.layers.MaxPooling2D((2, 2), strides=(2, 2))(conv5)
        conv6 = self.standard_unit(pooling5, filters[5])
        
        up1 = self.upsampling_block(conv6, filters[4])
        
        up1_padded = keras.layers.ZeroPadding2D(padding=((0, 1), (0, 1)))(up1)
        concat1 = keras.layers.Concatenate()([conv5, up1_padded])
        conv7 = self.standard_unit(concat1, filters[4])
        
        up2 = self.upsampling_block(conv7, filters[3])
        concat2 = keras.layers.Concatenate()([conv4, up2])
        conv8 = self.standard_unit(concat2, filters[3])
        
        up3 = self.upsampling_block(conv8, filters[2])
        concat3 = keras.layers.Concatenate()([conv3, up3])
        conv9 = self.standard_unit(concat3, filters[2])
        
        
        up4 = self.upsampling_block(conv9, filters[1])
        concat4 = keras.layers.Concatenate()([conv2, up4])
        conv10 = self.standard_unit(concat4, filters[1])
        
        up5 = self.upsampling_block(conv10, filters[0])
        concat5 = keras.layers.Concatenate()([conv1, up5])
        conv11 = self.standard_unit(concat5, filters[0])

        outputs = keras.layers.Conv2D(1, (1, 1), activation='sigmoid', padding='same', kernel_initializer='he_normal')(conv11)
