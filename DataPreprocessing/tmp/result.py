...
def calculate_best_worst(folder_name, result_path):
    ...
    f.writelines(lines)
    f.close()
    return  str(round(sum(arr_JI)/len(arr_JI), 3)),str(round(np.var(arr_JI), 3)) , str(round(sum(arr_DC)/len(arr_DC), 3)) ,str(round(np.var(arr_DC), 3)),best_case,str(round(best, 3)),worst_case, str(round(worst, 3))
...
def getResult(PATH):
    ...
    for dataset in path.iterdir():
        ...
        avg = {k: [round(sum(v[0])/len(v[0]), 3), round(sum(v[1])/len(v[1]), 3), round(sum(v[2])/len(v[2]), 3), round(sum(v[3])/len(v[3]), 3)] for k, v in avg.items()}
        df[dataset.name]['avg'] = avg
