...
# D. Jha et al., "ResUNet++: An Advanced Architecture for Medical Image Segmentation," 2019 IEEE International Symposium on Multimedia (ISM), San Diego, CA, USA, 2019, pp. 225-2255, doi: 10.1109/ISM46123.2019.00049.

'''
https://github.com/DebeshJha/ResUNetPlusPlus/blob/master/m_resunet.py
'''
class ResUNetPLUSPLUS(Model):
    def __init__(self, image_size, learning_rate):
      self.input_size = image_size
      self.learning_rate = learning_rate
      
    def squeeze_excite_block(self,inputs, ratio=4):
      init = inputs
      channel_axis = -1
    ...
    def stem_block(self,x, n_filter, strides):
      ...
      ## Conv 1
      x = keras.layers.Conv2D(n_filter, (3, 3), padding="same", strides=strides)(x)
      x = keras.layers.BatchNormalization()(x)
      x = keras.layers.Activation("relu")(x)
      x = keras.layers.Conv2D(n_filter, (3, 3), padding="same", strides=1)(x)

      ## Shortcut
      s  = keras.layers.Conv2D(n_filter, (1, 1), padding="same", strides=strides)(x_init)
    ...
    def aspp_block(self,x, num_filters, rate_scale=1):
      x1 = keras.layers.Conv2D(num_filters, (1, 1), padding="same")(x)
      x1 = keras.layers.BatchNormalization()(x1)

      x2 = keras.layers.Conv2D(num_filters, (3, 3), dilation_rate=(2 * rate_scale, 2 * rate_scale), padding="same")(x)
      x2 = keras.layers.BatchNormalization()(x2)

      x3 = keras.layers.Conv2D(num_filters, (3, 3), dilation_rate=(4 * rate_scale, 4 * rate_scale), padding="same")(x)
      x3 = keras.layers.BatchNormalization()(x3)
      
      x4 = keras.layers.Conv2D(num_filters, (3, 3), dilation_rate=(6 * rate_scale, 6 * rate_scale), padding="same")(x)
      x4 = keras.layers.BatchNormalization()(x4)

      y = keras.layers.Concatenate()([x1, x2, x3, x4])
      y = keras.layers.Conv2D(num_filters, (1, 1), padding="same")(y)
      return y
    ...
    def attetion_block(self,g, x):
      ...
      g_conv = keras.layers.BatchNormalization()(g)
      g_conv = keras.layers.Activation("relu")(g_conv)
      g_conv = keras.layers.Conv2D(filters, (1, 1), padding="same")(g_conv)

      g_pool = keras.layers.MaxPooling2D((2, 2))(g_conv)

      x_conv = keras.layers.BatchNormalization()(x)
      x_conv = keras.layers.Activation("relu")(x_conv)
      x_conv = keras.layers.Conv2D(filters, (1, 1), padding="same")(x_conv)

      gc_sum = keras.layers.Add()([g_pool, x_conv])

      gc_conv = keras.layers.BatchNormalization()(gc_sum)
      gc_conv = keras.layers.Activation("relu")(gc_conv)
      gc_conv = keras.layers.Conv2D(filters, (3, 3), padding="same")(gc_conv)

      gc_mul = keras.layers.Multiply()([gc_conv, x])
      return gc_mul
    ...
    def build_model(self, filters):
        ...
        c0 = inputs
        c1 = self.stem_block(c0, filters[0], strides=1)

        ## Encoder
        c2 = self.resnet_block(c1, filters[1], strides=2)
        c3 = self.resnet_block(c2, filters[2], strides=2)
        c4 = self.resnet_block(c3, filters[3], strides=2)

        ## Bridge
        b1 = self.aspp_block(c4, filters[4])

        ## Decoder
        d1 = self.attetion_block(c3, b1)
        d1 = keras.layers.UpSampling2D((2, 2))(d1)
        d1 = keras.layers.Concatenate()([d1, c3])
        d1 = self.resnet_block(d1, filters[3])

        d2 = self.attetion_block(c2, d1)
        d2 = keras.layers.UpSampling2D((2, 2))(d2)
        d2 = keras.layers.Concatenate()([d2, c2])
        d2 = self.resnet_block(d2, filters[2])

        d3 = self.attetion_block(c1, d2)
        d3 = keras.layers.UpSampling2D((2, 2))(d3)
        d3 = keras.layers.Concatenate()([d3, c1])
        d3 = self.resnet_block(d3, filters[1])

        ## output
        outputs = self.aspp_block(d3, filters[0])
        outputs = keras.layers.Conv2D(1, (1, 1), padding="same")(outputs)
